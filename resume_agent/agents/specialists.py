...
要求：
1. 必须贴合这段经历与岗位：参考「已确认事实」与「项目背景资料」（如有），选项应是用户在这段具体经历中最可能做出的回答；
2. 若提供了「项目背景资料」（如 GitHub README、项目简介、主要语言），选项必须引用其中的具体模块、功能或技术（例如「负责推荐排序模块的召回优化」），严禁生成「负责需求调研」「推动项目落地」这类任何项目都能套的空泛模板；
3. 覆盖不同侧重点（如不同动作、不同产出、不同规模），让用户总能找到接近自己情况的选项；
4. 用用户第一人称口吻（「我……」开头可不加，直接给短句）；
5. 不要包含任何解释、编号或括号说明。

只输出 JSON：{"options": ["选项1", ...]}"""

ROLE_OPTIONS_PROMPT = """你是简历导师。给定目标岗位、经历类型（实习/工作）与组织名称，生成 3~5 个用户在这段经历里最可能担任的岗位名（中文短标签，如「AI 产品实习生」「数据分析实习生」「运营专员」）。

要求：
1. 岗位名必须贴合经历类型：实习经历给实习生类岗位，工作经历给正式岗位；
2. 结合组织名称与目标岗位行业（如互联网公司可给「AI 产品实习生」「增长运营实习生」）；
3. 不要空泛（避免只输出「实习生」「员工」）。

只输出 JSON：{"options": ["岗位1", ...]}"""
